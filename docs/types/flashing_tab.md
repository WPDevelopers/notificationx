...
| **Link type** | `-1` (`$link_type`) — same sentinel used by `notification_bar`; meaning of `-1` not verified. `_TODO: verify_` |
...
3. The resulting `active`/`global` result is localized to the frontend (`window.notificationXArr`); the React runtime is responsible for reading the theme id (`flashing_tab_theme-1` … `-4`) and rendering the tab-flash behaviour client-side. `_TODO: verify_` exact JS entry point / hook (e.g. `useNotificationX.ts`) that consumes it.
...
Each theme also has a `source` key pointing at a static preview GIF under `NOTIFICATIONX_ADMIN_URL . 'images/extensions/themes/pro/flashing-tab/theme-N.gif'`. There is no separate `$res_themes` (responsive themes) entry for this extension in the reviewed code. `_TODO: verify_`
...
| Frontend runtime | `nxdev/notificationx/frontend/...` — `_TODO: verify_` exact component(s) that implement the tab-flash/favicon-swap behaviour |

## Dependencies

None required for the notification mechanism itself (pure browser `document.title`/favicon manipulation) — core WordPress + NotificationX PRO only. `_TODO: verify_` whether any theme-4 "cart quantity" default implies a WooCommerce (or similar) data dependency at runtime, since the reviewed PHP only hardcodes a placeholder message string (`{quantity} items in your cart!`) and does not itself fetch cart data.
...
- No dedicated test files for this Type were found under `tests/` in this pass (`grep -rli "flashing"` returned no hits). `_TODO: verify_`
- `_TODO: verify_` the JS/React component that actually swaps `document.title` and the `<link rel="icon">` href, and how it alternates between the two icon/message pairs on an interval.
