...
See [docs/exit-intent-popup.md](../exit-intent-popup.md) for the full trigger-mechanism and per-theme React rendering details, and [docs/exit-intent-add-new-design.md](../exit-intent-add-new-design.md) for how to add a new built-in theme.
...
- `exit_intent_cookie_days` — per [docs/exit-intent-popup.md](../exit-intent-popup.md), when > 0 a dismissal cookie is written in addition to the `sessionStorage` key.
...
- Per [docs/exit-intent-popup.md](../exit-intent-popup.md): two preview image URLs are intentionally cross-wired (`theme-six`'s registry entry points at the `theme-seven` PNG and vice versa) to match the admin picker's visual order — not a bug to "fix" without also updating the picker layout.
...
| Frontend runtime (React/TS) | `nxdev/notificationx/frontend/core/ExitIntentPopup.tsx`, `useNotificationX.ts`, `NotificationContainer.tsx`, `utils.ts` — see [docs/exit-intent-popup.md](../exit-intent-popup.md) for the full list |
...
- Adding/editing a built-in theme normally requires changes in multiple places — see [docs/exit-intent-add-new-design.md](../exit-intent-add-new-design.md) for the full checklist.
...
- [Adding a New Notification Type](../new-notification-type.md) — uses Exit Intent as its reference implementation
- [docs/exit-intent-popup.md](../exit-intent-popup.md) — deep dive: trigger mechanism, per-theme React rendering, shared CTA helper
- [docs/exit-intent-add-new-design.md](../exit-intent-add-new-design.md) — checklist for adding a new built-in theme
