...
There is also a second, near-duplicate implementation of the same exclude/include logic in the `Conversions` trait (`excludes_product`, `show_purchaseof` in [Traits/Conversions.php](../../includes/Types/Traits/Conversions.php)) with slightly different `array_diff` argument order than the class's own `_excludes_product`/`_show_purchaseof`. **Verified:** the trait's `excludes_product`/`show_purchaseof` have no call sites in either the free or the Pro plugin (only an `@todo` comment references them) — they are legacy/BC dead code; the wired-up methods are the class's own `_excludes_product`/`_show_purchaseof`.
...
3. The notification link is built from `$post['link_type']` at [FrontEnd.php:738](../../includes/FrontEnd/FrontEnd.php#L738) — Conversions declares `link_type = 'product_page'`, so unless it's `'none'` a link is attached. **Verified:** `link_type` is not copied onto the stored `$post`; it is resolved at read time by `Extension::get_link_type()` ([Extension.php:196](../../includes/Extensions/Extension.php#L196)), which uses the Extension's own `$link_type` and falls back to `$type->link_type` (the Type's `product_page`).
...
Depends on whichever data-source extension is active: WooCommerce, Easy Digital Downloads (EDD), SureCart, FluentCart, Envato, Freemius, Zapier (as a relay), or BitIntegrations. **Verified:** every one of these source-extension classes (WooCommerce, EDD, Envato, FreemiusConversions, SureCart, FluentCart, ZapierConversions, BitIntegrationsConversions) ships in the free plugin under `includes/Extensions/` — their availability is gated only by the `modules_*` settings key, not by Pro licensing (individual *themes* may still be `is_pro => true`).
...
- The trait's `excludes_product`/`show_purchaseof` duplicate similar logic to the class's `_excludes_product`/`_show_purchaseof` with different `array_diff` argument ordering; confirmed dead code in both free and Pro — the trait's `excludes_product`/`show_purchaseof` have no call sites anywhere.
- `conv-theme-fourteen`/`conv-theme-sixteen` (sales-count cards) depend on a Pro-side sales-count feature. **Verified in `notificationx-pro`:** the Pro `Conversions` type (`notificationx-pro/includes/Types/Conversions.php`) builds a `$sales_count_template` (`first_param => 'tag_sales_count'`) and assigns it to `conv-theme-seven/eight/nine/fourteen/sixteen`.
