...
| **Default theme** | _TODO: verify_ — `$default_theme` / `$default_res_theme` are inherited from `Types` (`''`) and never set in `PageAnalytics::init()` |
...
4. `includes/FrontEnd/FrontEnd.php` has no `page_analytics`-specific branch found in this pass (only generic `analytics_*` settings keys related to NotificationX's *own* usage-analytics opt-in, unrelated to this Type) — entries for this type flow through the same generic entry/render pipeline as other types. `_TODO: verify_` the exact REST/entries route that serves `page_analytics` notifications; no type-specific FrontEnd.php code path was found.
