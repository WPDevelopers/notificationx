...
| [Facebook](./facebook.md) | `modules_facebook_reviews` | reviews | Apify account + API token (`facebook_reviews_apify_token`); no local plugin to detect | Facebook Page Recommendations as Reviews popups, collected through the `apify/facebook-reviews-scraper` actor. Works in the free plugin; Pro adds translation, filtering and higher caps. |
