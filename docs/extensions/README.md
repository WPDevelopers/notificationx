...
| [Facebook Reviews](./facebook-reviews.md) | `modules_facebook_reviews` | reviews | Site owner's own Meta app (App ID/Secret) + HTTPS; no local plugin | Facebook Page aggregate rating plus individual recommendations (text, recommended/not, date — reviewer anonymous) as review notifications; free class is a stub, OAuth/Graph/cron in notificationx-pro. |
