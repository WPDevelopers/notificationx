...
| [LatePoint](./latepoint.md) | `modules_latepoint` | conversions | LatePoint — `class_exists('LatePoint')` | Confirmed LatePoint bookings as Sales notifications; buffered per order so recurring/cart checkouts emit one popup, with a daily reconciliation cron covering LatePoint's hookless deletion paths. |
