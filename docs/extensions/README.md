...
| [Facebook Reviews](./facebook-reviews.md) | `modules_facebook_reviews` | reviews | Site owner's own Meta app (App ID/Secret) + HTTPS; no local plugin | Facebook Page aggregate rating (`overall_star_rating`, `rating_count`) as a total-rated review notification; free class is a stub, OAuth/Graph/cron in notificationx-pro. |
