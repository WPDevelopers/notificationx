...
const RatingWidget = () => {
    ...
    const sendRating = async (rating) => {
        try {
            await nxHelper.get(`index.php?rest_route=/notificationx/v1/send-rating&rating=${rating}`);
        } catch (err) {
        ...
        <div className="notificationx-rating-widget sidebar-widget nx-widget">
            <>
                <div className="nx-widget-title"> 
                    <h4>{__('Share your feeling', 'notificationx')}</h4>
                    { !rating && 
                    ...
                    { ( (rating && rating < 5) || true) &&
                        <div className='nx-widget-review-box'>
                            <h4>{ __('Help us make it better!','notificationx') }</h4>
                            <div className="review-box">
                                <label htmlFor="review-box-desc">Description</label>
                                <textarea name="" id="review-box-desc"></textarea>
                            </div>
                            <button>{ __('Send','notificationx') }</button>
                        </div>
                    }
                    { ratingSubmitted &&
