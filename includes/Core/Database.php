...
class Database {
    ...
    public function __construct() {
        ...
        self::$table_maps      = $wpdb->prefix . 'nx_maps';
    }
    ...
    public function update_option( $key, $value, $autoload = 'no'){
        $is_exists = $this->wpdb->get_row("SELECT * FROM {$this->wpdb->prefix}options WHERE option_name='$key' LIMIT 1");
        if( $is_exists ) {
            ...
            $this->wpdb->insert("{$this->wpdb->options}", [ 'option_name' => $key, 'option_value' => $value ] );
        }
    }
